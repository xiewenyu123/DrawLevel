...
#define RECT_WIDTH 440      //绘制等级轴区域的宽度
#define RECT_HEIGHT 30      //绘制等级轴区域的高度
    ...
    if(m_datalist.size() < 10)
    {
        ...
        for(int i = 0;i < m_datalist.size();i++)
        {
            if(m_datalist[i].precent > (i + 1) * 0.05)
            {
                ...
                one.color = colorchoose->ColorName[i];
            }
        }
...
void DrawLevel::drawPrecentShow(QPainter &painter)
{
    ...
    if(m_datalist.size() == 1)  //绘制只有一个等级时的样式
    {
    ...
    {
        for(int i = 0; i < m_datalist.size();i++)   //大于一个等级时第一个的样式
        {
            if(i == 0)
            {
                //使用QPainterPath合并addRoundedRect和addRect的path形成目标path展现出矩形样式为一半圆角一半直角
                painter.setBrush(m_datalist[0].color);
                QPainterPath path_first;
                path_first.setFillRule( Qt::WindingFill );
                QRectF info_rect_first(0,2,RECT_WIDTH * m_datalist[0].precent,RECT_HEIGHT);
                path_first.addRoundedRect(QRectF(0,2,RECT_WIDTH * m_datalist[0].precent,RECT_HEIGHT),BORDER_RADIUS, BORDER_RADIUS);
                QRectF temp_rect_first(info_rect_first.left() + info_rect_first.width() / 2, info_rect_first.top(), info_rect_first.width() / 2, info_rect_first.height());
                path_first.addRect(temp_rect_first);
                painter.fillPath(path_first,m_datalist[0].color);

                painter.drawText(info_rect_first, Qt::AlignCenter, m_datalist[0].name);  //第一个绘制区域为圆角矩形



            }
            else if (i == m_datalist.size() - 1) {

                QPainterPath path_last;
                path_last.setFillRule( Qt::WindingFill );
                QRectF info_rect_last(RECT_WIDTH * m_datalist[i - 1].precent,2,RECT_WIDTH * (m_datalist[i].precent - m_datalist[i-1].precent),RECT_HEIGHT);
                path_last.addRoundedRect(QRectF(RECT_WIDTH * m_datalist[i - 1].precent,2,RECT_WIDTH * (m_datalist[i].precent - m_datalist[i-1].precent),RECT_HEIGHT),BORDER_RADIUS, BORDER_RADIUS);
                QRectF temp_rect_last(info_rect_last.left(), info_rect_last.top(), info_rect_last.width() / 2, info_rect_last.height());
                path_last.addRect(temp_rect_last);
                painter.fillPath(path_last,m_datalist[i].color);

                painter.drawText(info_rect_last, Qt::AlignCenter, m_datalist[0].name);  //第一个绘制区域为圆角矩形

                
            }
            else {
                QPainterPath path_center;
                path_center.setFillRule( Qt::WindingFill );
                QRectF temp_rect_center(RECT_WIDTH * m_datalist[i - 1].precent,2,RECT_WIDTH * (m_datalist[i].precent - m_datalist[i-1].precent),RECT_HEIGHT);
                path_center.addRect(temp_rect_center);
                painter.fillPath(path_center,m_datalist[i].color);

                painter.drawText(temp_rect_center, Qt::AlignCenter, m_datalist[i].name);
            }
...
void DrawLevel::mouseMoveEvent(QMouseEvent *event)
{
    ...
    for(int i = 0; i < m_datalist.size();i++)
    {
       ...
       QRectF rect = QRectF(QPointF(30 + m_Xpoint - TRIANGLE_WIDTH / 2,40 + 38),QSizeF(15,20)); //滑块范围
